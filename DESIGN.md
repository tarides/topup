...
- **Idle detection under concurrency.** Decided: `eval` returns as soon as the top-level expression returns. Background fibres (Lwt/Eio fibres, `Thread.create`, `Domain.spawn`) are the caller's responsibility — they keep running, are *not* killed by `reset` (OCaml exposes no thread-kill primitive), and their later writes to stdout/stderr may land in a subsequent `eval`'s capture or on the server's real stdout. Only process exit reliably clears them. True teardown awaits a future process-isolated session story (see the sandboxing item below). Documented in the `eval` MCP tool description and pinned by a `Domain.spawn` test in `test/test_session.ml`.
